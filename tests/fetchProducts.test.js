...
describe('1 - Teste a função fetchProducts', () => {
  it('Testa se fetchProducts é uma função', () =>  expect(typeof fetchProducts).toBe("function")); 

  it("Testa se, ao chamar a função fetchItem sem argumento, retorna um erro com a mensagem: 'You must provide an url'", async() => {
    const setElement = await fetchProducts()
    expect(setElement).toEqual(new Error('You must provide an url'));
  });
  ...
  it("Testa se o retorno da função fetchProducts com o argumento 'computador' é uma estrutura de dados igual ao objeto computadorSearch", async() => {
    const secondCase = await fetchProducts(endpoint)
    expect(secondCase).toEqual(computadorSearch.results);
  });
});
