...
describe('2 - Teste a função fetchItem', () => {
  it('Testa se fetchItem é uma função', () =>  expect(typeof fetchItem).toBe("function")); 

  it("Testa se a função fetchItem com o argumento 'MLB1615760527' e teste se fetch foi chamada", async () => {
  ...
  it("Testa se o retorno da função fetchItem com o argumento do item 'MLB1615760527' é uma estrutura de dados igual ao objeto item que já está importado no arquivo", async () => {
    const element = await fetchItem("MLB1615760527")
  ...
  it("Testa se, ao chamar a função fetchItem sem argumento, retorna um erro com a mensagem: 'You must provide an url'", async () => {
    const secondCase = await fetchItem()
    expect(secondCase).toEqual(new Error('You must provide an url'));
  });
